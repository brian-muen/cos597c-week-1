...
async function runComparison() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(data.error || "The comparison failed.");
...
async function loadResults() {
  try { const response = await fetch("/api/results"); const payload = await response.json(); if (!response.ok) throw new Error(payload.error); renderAccuracy(payload.statistics); renderPaired(payload.statistics.overall.paired); renderLatency(payload.statistics, payload.timings); renderVerification(payload.verification); } catch (error) { document.querySelector("#accuracy-chart").textContent = "Saved experiment results are unavailable."; document.querySelector("#verification-chart").textContent = "The focused verification results are unavailable."; }
}
